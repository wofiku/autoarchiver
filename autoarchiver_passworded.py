"""AutoArchiver Passworded - an automatic archive generator with password.
Generates an archive out of files in the same directory as script is.
Returns password in <first_file>.txt file, names archive as <first_file>.zip
...
def password_generator(length: int = 12) -> str:
    """Generates password in declared limit using letters and digits

    :param length: Length of password
    :return: Randomly generated password
    """

    logging.info("Started generating a new random password, please wait")

    # VARS
    length: int = length if isinstance(length, int) else ord(length)
    alphabet_with_digits: str = ascii_letters + digits
    password: str = ''.join(secrets_choice(alphabet_with_digits) for _ in range(length))

    logging.info("Random password is generated")
...
def create_archive_from_files(directory: str | Path = '.') -> None:
    ...
    # Getting password for future archive
    password: str = password_generator(12)
    password_filename: str = f"{archive_name}_password.txt"
    password_file_path: Path = directory / password_filename

    # MAIN
    # Creating archive of <first_file>.zip
    create_archive(files=files, arch_name=archive_name, password=password)
    # Creating a <first_file>.txt file with password
    with password_file_path.open('w', encoding='utf-8') as f:
        f.write(password)

    logging.info(f"Password has been saved in {password_file_path.resolve()}")
